...
- [x] **Rec 8** — whitepaper restructure — shipped 2026-07-08: prominent 'Implementation Status: Shipped vs Aspirational' section after the executive summary leads with what exists (intent verification, contracts + static contract lint, secure-by-default stack, gradual typing, agent diagnostics, concurrency/jobs/workers) and explicitly labels typed effects, session types, structured edits, semver enforcement, belief observability, workflow primitives, and UI constraints as future work; exec summary points to it
