...
- `set_local_password(identifier, current_password, new_password, options?) -> Result<LocalUser, String>` — shipped as the narrow setup-completion/password-rotation primitive; verifies the current credential, rejects same-password reuse, and returns the same safe local user payload shape as credential verification
