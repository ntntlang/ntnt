...
`traceroute(host, opts?)` traces the network path to a host with TTL-stepped native probes. The `method` option selects the protocol: `"icmp"` (default, echo requests, cross-platform), `"udp"` (datagrams to an unused port; reached on ICMP Port Unreachable), or `"tcp"` (SYNs to a real port; reached on SYN-ACK/RST — the variant most likely to pass firewalls). All methods **require a raw ICMP socket** for intermediate hops (usually `CAP_NET_RAW`; in Docker add `cap_add: [NET_RAW]`); `"udp"` and `"tcp"` are **Linux-only** (and `"tcp"` additionally needs a raw TCP socket). When the required capability is missing the call returns `Err(String)` rather than degrading — check the matching `net_capabilities()` flag first. All hops are probed concurrently and collected within the timeout, so a trace costs roughly one timeout instead of one per silent hop. Options: `method` (default `"icmp"`), `port` (default TCP 80 / UDP 33434), `max_hops` (default 30, max 64), `timeout_ms` (default 8000, a global budget across all hops), `allow_private`. To build mtr-style per-hop statistics, call `traceroute()` repeatedly and aggregate in app code (loss%, avg/best/worst RTT) — that composition is intentionally left to the application rather than the stdlib.
