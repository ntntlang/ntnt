...
The map contains `ping` (true when any ICMP echo path is available), `traceroute` (true when a raw ICMP path is available), `icmpv4_datagram`, `icmpv4_raw`, `icmpv6_datagram`, `icmpv6_raw`, and `tcp` booleans. It returns a plain `Map` rather than a `Result` because detection itself cannot fail.
