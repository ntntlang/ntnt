...
- Active `run` and `start` commands are reaped on runtime shutdown and direct CLI exits, started processes enforce deadlines and capture limits without caller polling, and detached descendants cannot hold capture finalization open indefinitely; Unix launches supervise descendant process groups, while Windows rejects implicit `.bat`/`.cmd` shell execution.
