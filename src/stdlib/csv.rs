...
pub fn init() -> HashMap<String, Value> {
    ...
    module.insert("parse".to_string(), Value::NativeFunction {
        ...
        func: |args| {
            let csv_string = match &args[0] {
                ...
                _ => return Err(IntentError::TypeError("csv.parse() requires a string".to_string())),
            };
    ...
    module.insert("parse_with_headers".to_string(), Value::NativeFunction {
        name: "csv_parse_with_headers".to_string(),
        arity: 1,
        func: |args| {
            let csv_string = match &args[0] {
                Value::String(s) => s.clone(),
                _ => return Err(IntentError::TypeError("csv.parse_with_headers() requires a string".to_string())),
            };
            ...
            let result: Vec<Value> = data_rows.iter()
                .map(|row| {
    ...
    module.insert("stringify".to_string(), Value::NativeFunction {
        name: "csv_stringify".to_string(),
        arity: 1,
        func: |args| {
            let rows = match &args[0] {
                Value::Array(arr) => arr,
                _ => return Err(IntentError::TypeError("csv.stringify() requires an array".to_string())),
            };
            ...
            for (i, row_value) in rows.iter().enumerate() {
                ...
                let row = match row_value {
                    Value::Array(arr) => arr,
                    _ => return Err(IntentError::TypeError("csv.stringify: each row must be an array".to_string())),
                };
    ...
    module.insert("stringify_with_headers".to_string(), Value::NativeFunction {
        name: "csv_stringify_with_headers".to_string(),
        arity: 2,
        func: |args| {
            let rows = match &args[0] {
                Value::Array(arr) => arr,
                _ => return Err(IntentError::TypeError("csv.stringify_with_headers() first arg must be array".to_string())),
            };
                ...
                Value::Array(arr) => {
                    arr.iter()
                        .map(|v| match v {
                            ...
                            _ => Err(IntentError::TypeError("Headers must be strings".to_string())),
                        })
                        .collect::<Result<Vec<_>, _>>()?
                },
                _ => return Err(IntentError::TypeError("csv.stringify_with_headers() second arg must be headers array".to_string())),
            };
            ...
            for row_value in rows.iter() {
                ...
                let row = match row_value {
                    Value::Map(m) => m,
                    _ => return Err(IntentError::TypeError("csv.stringify_with_headers: each row must be a map".to_string())),
                };
