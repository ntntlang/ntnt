...
#[test]
fn ct_subdomains_rejects_non_domain_input_as_result_error() {
    let (stdout, stderr, code) = run_ntnt_code(
        r#"
import { ct_subdomains } from "std/net"

match ct_subdomains("https://example.com/path", map { "timeout_ms": 100 }) {
    Ok(records) => print("unexpected"),
    Err(e) => print(e)
}
"#,
    ...
    assert_eq!(code, 0, "stderr: {stderr}\nstdout: {stdout}");
    assert!(stdout.contains("valid domain name"), "stdout: {stdout}");
}

#[test]
fn ct_subdomains_rejects_public_suffix_like_domains_as_result_error() {
    let (stdout, stderr, code) = run_ntnt_code(
        r#"
import { ct_subdomains } from "std/net"

match ct_subdomains("co.uk", map { "timeout_ms": 100 }) {
    Ok(records) => print("unexpected"),
    Err(e) => print(e)
}
"#,
    ...
    assert_eq!(code, 0, "stderr: {stderr}\nstdout: {stdout}");
    assert!(stdout.contains("public suffix-like"), "stdout: {stdout}");
}

#[test]
fn ct_subdomains_rejects_bad_options_as_result_error() {
    let (stdout, stderr, code) = run_ntnt_code(
        r#"
import { ct_subdomains } from "std/net"

match ct_subdomains("example.com", map { "max_results": 0 }) {
    Ok(records) => print("unexpected"),
    Err(e) => print(e)
}

match ct_subdomains("example.com", map { "max_results": 5001 }) {
    Ok(records) => print("unexpected"),
    Err(e) => print(e)
}
"#,
    ...
    assert_eq!(code, 0, "stderr: {stderr}\nstdout: {stdout}");
    assert!(
        stdout.contains("max_results must be between 1"),
        "stdout: {stdout}"
    );
}

#[test]
fn ct_subdomains_external_smoke_is_opt_in() {
    if std::env::var("NTNT_NET_EXTERNAL_CT_TESTS").as_deref() != Ok("1") {
        return;
    }
...
match ct_subdomains("iana.org", map { "timeout_ms": 30000, "max_results": 5 }) {
...
"#,
    ...
    assert_eq!(code, 0, "stderr: {stderr}\nstdout: {stdout}");
    assert!(!stdout.contains("ct="), "stdout: {stdout}");
}
