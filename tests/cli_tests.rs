...
fn test_validate_valid_file() {
    ...
    let json: serde_json::Value = serde_json::from_str(&stdout)
        .expect("validate should output valid JSON");
...
fn test_inspect_outputs_json() {
    ...
    let json: serde_json::Value = serde_json::from_str(&stdout)
        .expect("inspect should output valid JSON");
...
fn test_inspect_includes_line_numbers() {
    ...
    for func in functions {
        assert!(func["line"].is_number(), "Functions should have line numbers");
...
fn test_parse_json_flag() {
    ...
    let _: serde_json::Value = serde_json::from_str(&stdout)
        .expect("parse --json should output valid JSON");
}
...
fn test_validate_exits_nonzero_on_syntax_error() {
    ...
    assert_ne!(code, 0, "validate should exit with error code on syntax error");
