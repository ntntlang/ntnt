...
fn test_learn_stdout_outputs_rules() {
    ...
    assert!(
        stdout.contains("#{"),
        "Should mention interpolation syntax"
    );
}
...
fn test_learn_claude_code_creates_files() {
    ...
    assert_eq!(output.status.code().unwrap(), 0);
    assert!(&dir_path.join(".claude/CLAUDE.md").exists(), "Should create .claude/CLAUDE.md");
    assert!(&dir_path.join(".claude/rules/ntnt.md").exists(), "Should create .claude/rules/ntnt.md");

    // Verify content
    let claude_md = std::fs::read_to_string(&dir_path.join(".claude/CLAUDE.md")).unwrap();
    assert!(claude_md.contains("Generated by ntnt v"), "Should have version header");
    assert!(claude_md.contains("Critical Syntax Rules"), "Should have rules content");
}
...
fn test_learn_check_detects_stale() {
    ...
    let output = Command::new(&exe)
        ...
        .output()
        .expect("Failed to run ntnt learn --check");
    assert_eq!(output.status.code().unwrap(), 1, "Stale files should exit 1");
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_migrate_applies_changes() {
    ...
    let after = std::fs::read_to_string(&test_file).unwrap();
    assert_eq!(after, r#"let x = "Hello #{name}""#, "Should migrate old interpolation to new");
}
...
fn test_migrate_preserves_route_params() {
    ...
    let after = std::fs::read_to_string(&test_file).unwrap();
    assert_eq!(after, r#"get("/users/{id}", handler)"#, "Route params should not be migrated");
}
...
fn test_migrate_skips_template_strings() {
    ...
    let after = std::fs::read_to_string(&test_file).unwrap();
    assert_eq!(after, r#"let x = """hello {{name}}""""#, "Template strings should not be modified");
}
